    ...
    nav_panel(
      ...
      sidebarLayout(
        sidebarPanel(
          ...
          downloadButton(ns("download_db"), "Download database"),
          div(style = "margin-bottom:20px"),
          downloadButton(ns("download_tag_cat"), "Download tag categories"),
          div(style = "margin-bottom:20px"),
          uiOutput(ns("download_citation"))
        ),
        mainPanel(width = 9, DTOutput(ns("table_full")))
      )
    ),
    ## Search db panel ---------------------
    nav_panel(
      ...
      layout_columns(
        ...
        card(
          card_header(h3("Paper table (filtered)")),
          card_body(
            checkboxInput(
              ns("show_extra"),
              "Show \"extra\" field",
        ...
        card(
          card_header(h3("Search criteria")),
          input_task_button(ns("clear_all_criteria"), "Clear all criteria"),
          checkboxInput(
            ns("exclude_obsolete"),
          ...
          card(card_header(h4("Notes criteria")), uiOutput(ns("notes"))),
          card(
            card_header(h4("Tag criteria")),
      ...
      sidebarLayout(
        sidebarPanel(
          ...
          radioButtons(
            ns("plot_data"),
            "Plot data",
            choices = c("Full dataset", "Filtered dataset"),
          ...
          radioButtons(
            ns("show_bar_data"),
            "Show bar data value",
            choices = c("None", "Percent", "Number of papers"),
            selected = "None"
          ),
          downloadButton(ns("download_plot_data"), "Download plot data"),
          HTML("<br>"),
          HTML("<br>"),
          card(
            card_header("Selection criteria"),
            DTOutput(ns("criteria_table_plot"))
          )
        ),
        mainPanel(width = 9, h3("Plot"), plotOutput(ns("plot")))
      )
    ),
    ## Summary table panel ---------------------------
    nav_panel(
      ...
      sidebarLayout(
        ...
        mainPanel(width = 9, DTOutput(ns("summary_table")))
      )
    ),
    ## Reports panel --------------------------
    nav_panel(
      ...
      sidebarLayout(
        sidebarPanel(
          width = 3,
          textInput(ns("report_title"), "Report title:", value = "untitled"),
          textInput(ns("report_author"), "Report author:", value = ""),
          ...
          selectInput(
            ns("report_sort_order"),
            "Report sort order",
            choices = c("author", "publication_year", "title"),
            selected = "author"
          ),
          checkboxGroupInput(
            ns("report_include"),
          ...
          radioButtons(
            ns("report_type"),
            "Report download file type:",
            choices = c("html", "pdf", "docx"),
            selected = "html"
          ),
          downloadButton(ns("download_report"), "Download report")
        ),
        mainPanel(width = 9, htmlOutput(ns("report"), style = "width: 100%;"))
      )
    ),
    ## Help panel ------------------------
    nav_panel(
