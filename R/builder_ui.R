...
builder_ui <- function(id) {
  ...
  page_navbar(
    ## App title --------------------------------
    title = div(
      img(src = "www/lit_tag_logo_1.png", height = "100px"),
      h1("Builder"),
      HTML("&nbsp; &nbsp;"),
    ...
    nav_panel(
      ...
      layout_columns(
        ...
        card(
          card_header(h3("Paper info and notes")),
          checkboxInput(
            ns("remove_timestamps"),
            "Remove timestamps from original file name",
            value = TRUE,
            width = "100%"
          ),
          downloadButton(ns("download_edits"), "Save edits"),
        ...
        ### Right panel: tags -------------------------------------------
        card(card_header(h3("Tags")), navset_pill(id = ns("tag_tabs")))
      )
    ),
    ...
    nav_panel(
      ...
      sidebarLayout(
        sidebarPanel(
          ...
          fileInput(
            ns("cat_new_db"),
            h4("Categories for new db"),
            multiple = FALSE,
            accept = c(".xlsx")
          ),
          textInput(ns("new_db_name"), "New database name", value = "Untitled"),
          downloadButton(ns("new_database"), "Download new database")
        ),
        mainPanel(
          width = 9,
    ...
    nav_panel(
      ...
      sidebarLayout(
        sidebarPanel(
          width = 3,
          fileInput(
            ns("sync_database_csv"),
            h4("Database to sync"),
            multiple = FALSE,
            accept = c(".csv")
          ),
          fileInput(
            ns("sync_categories_excel"),
            h4("Categories file for sync"),
            multiple = FALSE,
            accept = c(".xls", ".xlsx")
          ),
          fileInput(
            ns("sync_zotero_csv"),
            h4("Zotero csv file for sync"),
            multiple = FALSE,
            accept = c(".csv")
          ),
          downloadButton(
            ns("update_from_zotero"),
    ...
    nav_panel(
      h2("Database Maintenance"),
      tabsetPanel(
        id = ns("db_maintenance_panel"),
        ### Database contents ------------------------------------------
        tabPanel(
          "Database contents",
          sidebarLayout(
            ...
            mainPanel(width = 9, DT::DTOutput(ns("db_tags_table")))
          )
        ),
        ### Compare databases -------------------------
        tabPanel(
          ...
          sidebarLayout(
            sidebarPanel(
              ...
              input_task_button(ns("compare_db"), "Compare databases"),
        ...
        tabPanel(
          ...
          h3(
            "These options download a new database with
                                                        the selected change"
          ),
          navset_pill_list(
            nav_panel(
              "Replace tag name",
              textInput(ns("old_tag_name"), "Old tag name: "),
              textInput(ns("new_tag_name"), "New tag name: "),
              downloadButton(
                ns("replace_tag_name_download"),
                "Download database with replaced tag name"
              )
            ),
            nav_panel(
              "Replace tag option name",
              textInput(ns("tag_name"), "Tag name: "),
              textInput(ns("old_option_name"), "Old option name: "),
              textInput(ns("new_option_name"), "New option name: "),
              downloadButton(
                ns("replace_option_name_download"),
            ...
            nav_panel(
              ...
              fileInput(
                ns("zotero_for_delete"),
                h4("Zotero for delete comparison"),
                multiple = FALSE,
                accept = c(".csv")
              ),
              downloadButton(
                ns("delete_not_in_zotero_download"),
            ...
            nav_panel(
              ...
              textInput(
                ns("delete_papers_with_tag_options"),
                "Tag options for paper deletion (specify as \"tag/option\", comma seperated): "
              ),
              downloadButton(
                ns("delete_papers_with_tag_option_download"),
        ...
        tabPanel(
          ...
          sidebarLayout(
            sidebarPanel(
              ...
              downloadButton(
                ns("download_combined"),
                "Download combined database"
              )
            ),
            mainPanel(width = 9, h3(htmlOutput(ns("n_combined"))))
          )
        )
      )
    ),
    ...
    nav_panel(
      ...
      sidebarLayout(
        sidebarPanel(
          ...
          radioButtons(
            ns("all_or_unique_ris"),
            "RIS file",
            choices = c("All references", "Unique titles"),
      ...
      navset_pill(
        ...
        nav_panel(
          ...
          sidebarLayout(
            sidebarPanel(
              width = 3,
              downloadButton(ns("unicorn_example"), "Unicorn db example files"),
              HTML("<br>"),
              HTML("<br>"),
              downloadButton(ns("mcdr_example"), "mCDR db example files")
            ),
            mainPanel(width = 9)
          )
        )
