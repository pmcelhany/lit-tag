#' The lit-tag-builder ui
#'
#' @param id module ID.
#'
#' @import shiny
#' @import shinyWidgets
#' @import waiter
#' @import bslib
#' @import DT
#' @noRd

# ui ----------------------
builder_ui <- function(id){
  ns <- NS(id)
  page_navbar(
    ## App title --------------------------------
    title = div(img(src = "www/lit_tag_logo_1.png", height = "100px"),
                h1("Builder"), HTML("&nbsp; &nbsp;"),
                style = "color: brown; display: flex; align-items: center;"),

    ## Tag edit panel ---------------------
    nav_panel(h2("Tag edit"),
              tags$head(
                tags$link(rel = "stylesheet", type = "text/css", href = "www/styles.css")
              ),
              useWaiter(),
              layout_columns(
                col_widths = c(4,4,4),
                ### Left panel: database path and table of papers ---------------------
                card(card_header(h3("Paper table")),
                     style = "border",
                     fileInput("database_csv", h4("Database File"),
                               multiple = FALSE, accept = c(".csv")),
                     fileInput("categories_excel", h4("Categories File"),
                               multiple = FALSE, accept = c(".xls", ".xlsx")),
                     input_task_button("load_data", "Load database"),
                     textOutput("n_db"),
                     textOutput("n_filtered_db"),
                     #input_task_button("unselect_filters", "Unselect all filters"),
                     checkboxInput("exclude_obsolete", "Exclude obsolete papers",
                                   value = TRUE),
                     virtualSelectInput(
                       inputId = "filter_var",
                       label = "Filter variables",
                       choices = "",
                       hideClearButton = FALSE,
                       multiple = TRUE,
                       disableOptionGroupCheckbox = TRUE
                     ),
                     uiOutput("filters"),
                     input_task_button("filter_db", "Filter database"),
                     input_task_button("show_all_db", "Show all papers"),
                     DTOutput('table')),

                ### Center panel: paper info and notes -----------------------------------
                card(card_header(h3("Paper info and notes")),
                     #input_task_button("save_edits", "Save Edits"),
                     downloadButton("download_edits",
                                    "Save edits"),
                     #bsButton("save_edits", "Save Edits", style = "primary"),
                     textOutput("selected_author"),
                     textOutput("selected_year"),
                     textOutput("selected_title"),
                     textOutput("selected_journal"),
                     input_task_button("show_abstract", "Abstract"),
                     uiOutput("notes")),
                ### Right panel: tags -------------------------------------------
                card(card_header(h3("Tags")),
                     navset_pill(id = "tag_tabs"))
              )
    ),

    ## Sync zotero panel ---------------------
    nav_panel(h2("Sync Zotero"),
              sidebarLayout(
                sidebarPanel(width = 3,
                             fileInput("zotero_csv", h4("Zotero CSV File"),
                                       multiple = FALSE, accept = c(".csv")),
                             downloadButton("update_from_zotero",
                                            "Update database from Zotero")
                ),
                mainPanel(width = 9,
                          textOutput("n_init_db"),
                          textOutput("n_zotero"),
                          textOutput("n_new_keys"),
                          textOutput("n_old_key"),
                          textOutput("n_new_db")
                )

              )
    ),
    ## Database maintenance panel --------------------------------
    nav_panel(h2("Database Maintenance"),
              tabsetPanel(id = "db_maintenance_panel",
                          ### Database contents ------------------------------------------
                          tabPanel("Database contents",
                                   sidebarLayout(
                                     sidebarPanel(width = 3,
                                                  h3("This tab shows all contents of db, not just options from categories file"),
                                                  HTML("<br>"),
                                                  fileInput("content_db",
                                                            h4("Database for evaluation"),
                                                            multiple = FALSE,
                                                            accept = c(".csv")),
                                                  HTML("<br>"),
                                                  h4("Click on table row to see unique tag values"),
                                                  HTML("<br>"),
                                                  htmlOutput("n_papers"),
                                                  HTML("<br>"),
                                                  HTML("<b>Noted fields</b>"),
                                                  DTOutput("db_notes_table")),
                                     mainPanel(width = 9,
                                               DTOutput("db_tags_table")))),
                          ### Compare databases -------------------------
                          tabPanel("Compare databases",
                                   sidebarLayout(
                                     sidebarPanel(width = 3,
                                                  fileInput("compare_db_1",
                                                            h4("Database to compare #1"),
                                                            multiple = FALSE,
                                                            accept = c(".csv")),
                                                  fileInput("compare_db_2",
                                                            h4("Database to compare #2"),
                                                            multiple = FALSE,
                                                            accept = c(".csv")),
                                                  input_task_button("compare_db",
                                                                    "Compare databases"),
                                                  htmlOutput("n_papers_compare_1"),
                                                  htmlOutput("n_papers_compare_2")),
                                     mainPanel(width = 9,
                                               h3("Papers in db #1, but not in db #2"),
                                               DTOutput("papers_in_1_not_2"),
                                               h3("Papers in db #2, but not in db #1"),
                                               DTOutput("papers_in_2_not_1")))),
                          ### Replace/delete data -----------------
                          tabPanel("Replace/delete data",
                                   fileInput("edit_db",
                                             h4("Database to replace/delete"),
                                             multiple = FALSE,
                                             accept = c(".csv")),
                                   h3("These options download a new database with
                                                        the selected change"),
                                   navset_pill_list(
                                     nav_panel("Replace tag name",
                                               textInput("old_tag_name",
                                                         "Old tag name: "),
                                               textInput("new_tag_name",
                                                         "New tag name: "),
                                               downloadButton("replace_tag_name_download",
                                                              "Download database with replaced tag name")),
                                     nav_panel("Replace tag option name",
                                               textInput("tag_name",
                                                         "Tag name: "),
                                               textInput("old_option_name",
                                                         "Old option name: "),
                                               textInput("new_option_name",
                                                         "New option name: "),
                                               downloadButton("replace_option_name_download",
                                                              "Download database with replaced tag option name")),
                                     nav_panel("Delete tags",
                                               textInput("delete_tags",
                                                         "Tags to delete (comma seperated): "),
                                               downloadButton("delete_tags_download",
                                                              "Download database with deleted tags")),
                                     nav_panel("Delete tag options",
                                               textInput("delete_tag_options",
                                                         "Tag options to delete (specify as \"tag/option\", comma seperated): "),
                                               downloadButton("delete_tag_option_download",
                                                              "Download datase with deleted tag options")),
                                     nav_panel("Delete papers not in zotero",
                                               fileInput("zotero_for_delete",
                                                         h4("Zotero for delete comparison"),
                                                         multiple = FALSE,
                                                         accept = c(".csv")),
                                               downloadButton("delete_not_in_zotero_download",
                                                              "Download database with papers not in Zotero deleted")),
                                     nav_panel("Delete papers based on tag options",
                                               textInput("delete_papers_with_tag_options",
                                                         "Tag options for paper deletion (specify as \"tag/option\", comma seperated): "),
                                               downloadButton("delete_papers_with_tag_option_download",
                                                              "Download database with paper deletion based on tag options"))
                                   )
                          )
              )),

    ## New zotero panel ---------------------
    nav_panel(h2("New Zotero"),
              sidebarLayout(
                sidebarPanel(width = 3,
                             fileInput("database_nz_csv", h4("Old Keys Database File"),
                                       multiple = FALSE, accept = c(".csv")),
                             downloadButton("generate_ris", "Generate RIS"),
                             HTML("<br>"),
                             HTML("<br>"),
                             fileInput("database_knz_csv", h4("New Keys Zotero File"),
                                       multiple = FALSE, accept = c(".csv")),
                             downloadButton("generate_new_keys_db",
                                            "Generate new keys database")
                ),
                mainPanel(width = 9,
                          textOutput("n_old_key_db"),
                          textOutput("ris_generated"),
                          textOutput("n_new_key_zotero"),
                          textOutput("n_new_key_db"),
                          textOutput("n_new_key_db_missing")
                )
              )
    ),

    ## Help panel ------------------------
    nav_panel(h2("Help"),
              navset_pill(
                nav_panel("lit-tag-builder",
                          tags$iframe(style="height:100vh; width:100%; scrolling=yes",
                                      src="www/lit-tag-builder user guide.pdf")),
                nav_panel("example files",
                          sidebarLayout(
                            sidebarPanel(width = 3,
                                         downloadButton("unicorn_example",
                                                        "Unicorn db example files"),
                                         HTML("<br>"),
                                         HTML("<br>"),
                                         downloadButton("mcdr_example",
                                                        "mCDR db example files")
                            ),
                            mainPanel(width = 9)
                          )
                )
              )
    )
  )
}
